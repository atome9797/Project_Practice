...
public class GoldKeyAnimationController : MonoBehaviour
{
    [SerializeField]
    ...
    private void CheckItem()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hitInfo, range, layerMask))
        {
            ...
            if (hitInfo.transform.tag == "GoldKeyDoor")
            {
                ItemInfoAppear();
            }
        }
    ...
    private void CanPickUp()
    {
        if (pickActivated)
        {
            if (hitInfo.transform != null)
            {
                ...
                //문일때 열리는 애니메이션 실행
                if (hitInfo.transform.GetComponent<ItemPickUp>().item.itemName == "GoldDoor")
                {
                    if (key != null)
                    {
